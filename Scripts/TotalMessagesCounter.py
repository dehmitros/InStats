...
messages_folder = os.path.join(path)

for folder_name in os.listdir(messages_folder):
    folder_path = os.path.join(messages_folder, folder_name)
    if not os.path.isdir(folder_path):
        continue

    message_files = []
    for message_file_name in os.listdir(folder_path):
        if message_file_name in ['message_1.json', 'message_2.json, message_3.json']:
...
with open('TotalMessages.txt', 'w') as f:
